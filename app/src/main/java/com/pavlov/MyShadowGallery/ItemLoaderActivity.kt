...
import com.pavlov.MyShadowGallery.util.startSmallLoadingIndicator
import com.pavlov.MyShadowGallery.util.stopSmallLoadingIndicator
//import com.pavlov.MyShadowGallery.util.smallLoadingIndicator
...
class ItemLoaderActivity : AppCompatActivity() {
    ...
    private lateinit var loadingIndicator4: ProgressBar
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        loadingIndicator4 = findViewById(R.id.loading_indicator4)
...
//                    buttonsForImageDialog?.visibility = View.VISIBLE
            ...
            if (encryptedFileName.endsWith(".o", true) || encryptedFileName.endsWith(
                ...
                performShowLikeItIs()
                showShareImageDialog(context, encryptedFile)
...
//                        loadingIndicator3
    ...
    fun showShareImageDialog(context: Context, encryptedFile: File) {
        val builder = AlertDialog.Builder(context)
        builder.setTitle(context.getString(R.string.saving_option))
        activity.startSmallLoadingIndicator()
...
//                    var outputFile = createFileFrom(encryptedFile, ".k")

// Проверяем, находится ли файл в нужной папке
//                    if (outputFile != null) {
//                        if (!outputFile.parentFile?.absolutePath.equals(context.applicationContext.filesDir.absolutePath)) {
//                            // Если не находится, перемещаем файл в нужную папку
//                            val newFile = File(context.applicationContext.filesDir, outputFile.name)
...
//                    }
                        ...
                        try {
                            ...
                            when (selectedValue) {
                                ...
                                4 -> {
                                    ...
                                    if (APKM(context).getDefauldKey().isBlank()) {
                                        justEncryptByUserKey(
                                            encryptedFile,
                                            newEncryption,
                                            newDecryption
                                        )
    ...
    private fun encryptAndDerryptByKey(
        encryptedFile: File,
        newEncryption: String,
        newDecryption: String
    ) {
        ...
        MainScope().launch {
            try {
                ...
                activity.showToast(context.getString(R.string.error0))
                activity.stopSmallLoadingIndicator()
            } finally {
                activity.stopSmallLoadingIndicator()
            }
            closeContext2()
    ...
    private fun justEncryptByUserKey(
        encryptedFile: File,
        newEncryption: String,
        newDecryption: String
    ) {
...
//            activity.showLoadingIndicator()// в фоновом потоке, Корутина
            activity.showToast(context.getString(R.string.wait))
            val newFileName = "${encryptedFile.nameWithoutExtension}.o"
            val imageFile =
                FileProviderAdapter.bitmapToFileByKorutin(
                    rotatedBitmap,
                    context.applicationContext,
            ...
            val fileToDelete =
                File(context.applicationContext.filesDir, encryptedFile.name)
            if (fileToDelete.exists()) {
                fileToDelete.delete()
            }
            closeContext2()
